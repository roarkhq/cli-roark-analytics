...
export const GROUPS: Readonly<Record<string, string>> = {
  ...
  'autoimprove fix': 'Manage autoimprove fixes',
  'autoimprove fix guidance': 'Read autoimprove fix guidance',
...
export const COMMANDS: readonly CliCommand[] = [
  ...
  {
    commandPath: ['autoimprove', 'fix', 'answer'],
    clientProperty: 'autoimproveFix',
    methodName: 'answerQuestion',
    httpMethod: 'post',
    httpPath: '/v1/autoimprove/fix/{fixId}/answer',
    summary: "Answer Roark's question",
    description:
      'Answer the question a fix is blocked on. Only meaningful while the fix status is NEEDS_INPUT (the open QUESTION entry carries the offered options; free text is also accepted). Otherwise returns a conflict.',
    positionals: [
      {
        name: 'fix-id',
        paramKey: 'fixId',
  ...
  {
    commandPath: ['autoimprove', 'fix', 'cancel'],
    clientProperty: 'autoimproveFix',
    methodName: 'cancel',
    httpMethod: 'post',
    httpPath: '/v1/autoimprove/fix/{fixId}/cancel',
    summary: 'Cancel a fix',
    description:
      'Stop a live fix. Production is never changed by a cancel; everything the fix set up (the shadow agent, its phone number, authored test flows and run plan) is cleaned up automatically.',
    positionals: [
      {
        name: 'fix-id',
        paramKey: 'fixId',
  ...
  {
    commandPath: ['autoimprove', 'fix', 'create'],
    clientProperty: 'autoimproveFix',
    methodName: 'create',
    httpMethod: 'post',
    httpPath: '/v1/autoimprove/fix',
    summary: 'Start an Autoimprove fix',
    description:
      'Commission Roark on an objective. It clones the agent into a staging shadow (or uses your designated staging agent), authors a validation suite of simulated callers, measures a baseline, changes the staging configuration, and re-tests until the objective metric passes its target. Production is never touched by the loop; a verified fix waits for promotion. Requires an active provider integration (Vapi or Retell) with agent config writes enabled. One live fix per agent: starting a second returns a conflict. The fix runs asynchronously; poll GET /v1/autoimprove/fix/{fixId} or watch it in the dashboard. When its status is NEEDS_INPUT, answer via the answer endpoint; when AWAITING_PROMOTE, promote or dismiss.',
    positionals: [],
    flags: [
      {
        name: 'agent-id',
      ...
      },
      {
        ...
        location: 'body',
        required: true,
        description: 'Human-readable label for the objective, shown everywhere the fix appears.',
        valueKind: 'string',
        repeatable: false,
      },
      {
      ...
      },
      {
        ...
        location: 'body',
        required: false,
        description: 'Cap on decision turns. Defaults to 10.',
        valueKind: 'integer',
        repeatable: false,
      },
      {
  ...
  {
    commandPath: ['autoimprove', 'fix', 'dismiss'],
    clientProperty: 'autoimproveFix',
    methodName: 'dismiss',
    httpMethod: 'post',
    httpPath: '/v1/autoimprove/fix/{fixId}/dismiss',
    summary: 'Dismiss a verified fix',
    description:
      'Discard a verified fix without promoting: production stays untouched and the staging resources are cleaned up. Only a fix in AWAITING_PROMOTE can be dismissed.',
    positionals: [
      {
        name: 'fix-id',
        paramKey: 'fixId',
  ...
  {
    commandPath: ['autoimprove', 'fix', 'get'],
    clientProperty: 'autoimproveFix',
    methodName: 'getByID',
    httpMethod: 'get',
    httpPath: '/v1/autoimprove/fix/{fixId}',
    summary: 'Get an Autoimprove fix',
    description:
      'Fetch one fix with its full worklog: every step Roark took, the validation batches with their pass-rate movement, any question it is waiting on, and its final report once concluded.',
    positionals: [
      {
        name: 'fix-id',
        paramKey: 'fixId',
  ...
  {
    commandPath: ['autoimprove', 'fix', 'guidance', 'send'],
    clientProperty: 'autoimproveFix',
    methodName: 'sendGuidance',
    httpMethod: 'post',
    httpPath: '/v1/autoimprove/fix/{fixId}/guidance',
    summary: 'Send guidance to a running fix',
    description:
      'Steer Roark mid-fix. The message is folded into its next decision and is binding. Accepted while the fix is live (RUNNING, NEEDS_INPUT, or PAUSED); a concluded fix returns a conflict.',
    positionals: [
      {
        name: 'fix-id',
        paramKey: 'fixId',
  ...
  {
    commandPath: ['autoimprove', 'fix', 'list'],
    clientProperty: 'autoimproveFix',
    methodName: 'list',
    httpMethod: 'get',
    httpPath: '/v1/autoimprove/fix',
    summary: 'List Autoimprove fixes',
    description:
      'List the Autoimprove fixes in this project, most recent first, capped at 100. A fix is one autonomous engagement: Roark improving one agent toward one objective metric on a staging copy, with a human-gated promote to production at the end.',
    positionals: [],
    flags: [],
    hasParams: false,
    paramsAllOptional: true,
  ...
  {
    commandPath: ['autoimprove', 'fix', 'promote'],
    clientProperty: 'autoimproveFix',
    methodName: 'promote',
    httpMethod: 'post',
    httpPath: '/v1/autoimprove/fix/{fixId}/promote',
    summary: 'Promote a verified fix to production',
    description:
      'Apply the verified staging changes to the PRODUCTION agent. Only a fix in AWAITING_PROMOTE can be promoted. A snapshot of the production configuration is taken immediately before the write, so the promote is fully rollbackable. After the promote the staging shadow and its phone number are cleaned up.',
    positionals: [
      {
        name: 'fix-id',
        paramKey: 'fixId',
